...
// Generalized reorder-the-list. Like build_loop but for plain text items
// (algorithms, time complexities, steps of a process) rather than code blocks.
export default function OrderStepsQ({ q, locked, picked, onSubmit }) {
  // `order` holds ORIGINAL indices in the user's current arrangement.
  // [0,1,2,...] means in-order (correct).
  const [order, setOrder] = useState(() => {
    const idx = q.items.map((_, i) => i);
    let shuffled = shuffle(idx);
    // Cosmetically guarantee at least one swap from the correct order so
    // the student always has work to do.
        ...
        {order.map((origIdx, i) => {
          ...
          return (
            <div key={i} className={cls}>
              ...
              <div className="qz-block-text qz-step-text">
